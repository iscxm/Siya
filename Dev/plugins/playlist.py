...
@lang.language()
async def list_cmd(_, m: types.Message):
    try:
        await m.delete()
    except Exception:
        pass
...
async def plsongs_cmd(_, m: types.Message):
    if len(m.command) < 2:
        return await m.reply_text("Usage: <code>/view list_name</code>")
...
@app.on_message(filters.command(["remove"]) & ~app.bl_users)
@lang.language()
async def plremove_cmd(_, m: types.Message):
    if len(m.command) < 3 or not m.command[-1].isdigit():
        return await m.reply_text("Usage: <code>/remove playlist_name position</code>")
    pos = int(m.command[-1])
...
@app.on_message(filters.command(["delete"]) & ~app.bl_users)
@lang.language()
async def pldel_cmd(_, m: types.Message):
    if len(m.command) < 2:
        return await m.reply_text("Usage: <code>/delete playlist_name</code>")
    name = " ".join(m.command[1:])
...
async def spotify_cmd(_, m: types.Message):
    ...
    async def process_track(sp):
        query = f"{sp['name']} {sp.get('artists', '')}".strip()
        return await yt.search(query, sent.id)
        
    chunk_size = 15
    for i in range(0, len(sp_tracks), chunk_size):
        chunk = sp_tracks[i:i+chunk_size]
        tasks = [process_track(sp) for sp in chunk]
        results = await asyncio.gather(*tasks, return_exceptions=True)
        
        for track in results:
            if isinstance(track, Exception) or not track:
                continue
...
@app.on_message(filters.command(["cp", "copy"]) & ~app.bl_users)
@lang.language()
async def cp_cmd(_, m: types.Message):
    ...
    if m.reply_to_message:
        ...
        if not args:
            msg = await m.reply_text("<emoji id='5273914604752216432'>⚠️</emoji> Reply to a user's message or provide their username/id.")
            return msg.delete(delay=10)
        try:
            target_user = await app.get_users(args[0])
            target_name = " ".join(args[1:]).strip() if len(args) > 1 else None
        except Exception:
            msg = await m.reply_text("<emoji id='5420323339723881652'>❌</emoji> Could not find that user.")
            return msg.delete(delay=10)
    ...
    if target_user.id == m.from_user.id:
        msg = await m.reply_text("<emoji id='5420323339723881652'>❌</emoji> You can't copy from yourself, genius.")
        return msg.delete(delay=10)

    sent = await m.reply_text("<emoji id='5971801057540443125'>⚡</emoji> Extracting data...")
    
    target_playlists = await db.get_playlists(target_user.id)
    if not target_playlists:
        await sent.edit_text("<emoji id='5420323339723881652'>❌</emoji> Target has no playlists to copy.")
        async def _del1():
            import asyncio
        ...
        import asyncio
        asyncio.create_task(_del1())
        return
        
    saved = 0
    if target_name:
        if target_name not in target_playlists:
            await sent.edit_text("<emoji id='5420323339723881652'>❌</emoji> Requested playlist not found.")
            async def _del2():
                import asyncio
                await asyncio.sleep(10)
                try: await sent.delete()
    ...
    await sent.edit_text(
        f"<emoji id='5978776462386269988'>✨</emoji> <b>Copy Complete!</b>\n\n"
        f"<b>Target:</b> {target_user.mention}\n"
        f"<b>Copied:</b> <b>{saved}</b> songs successfully."
    )
    async def _del3():
        import asyncio
    ...
    import asyncio
    asyncio.create_task(_del3())


@app.on_message(filters.command(["list"]) & ~app.bl_users)
@lang.language()
