...
async def _run_speedtest():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        # ── Download (max 2 seconds) ─────────────────────────────────────────
        dl_start = time.monotonic()
        dl_bytes = 0
        try:
            async with session.get(DL_URL, timeout=aiohttp.ClientTimeout(total=5)) as r:
                async for chunk in r.content.iter_chunked(256 * 1024):  # 256KB chunks
                    dl_bytes += len(chunk)
