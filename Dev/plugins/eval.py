...
from Dev import unnati, app, config, db, lang, userbot
from Dev.helpers import format_exception, meval
...
async def eval_handler(_, message: types.Message):
    ...
    async def _eval_code() -> Tuple[str, Optional[str]]:
        ...
        try:
            ...
            return "error", formatted_tb

    state, result = await _eval_code()
    ...
    if is_error:
        header = "<b>\U0001f4a5 Eval Error</b>"
        body = (
            f"<b>Code:</b>\n<pre>{escape(code[:500])}</pre>\n\n"
            f"<b>Traceback:</b>\n<pre>{escape(output)}</pre>"
        )
    else:
        header = "<b>\u2705 Eval Output</b>"
        body = (
            f"<b>Code:</b>\n<pre>{escape(code[:500])}</pre>\n\n"
            f"<b>Result:</b>\n<pre>{escape(output) if output else '(no output)'}</pre>"
        )
