...
async def get_next_autoplay_track(chat_id: int, last: dict) -> dict | None:
    ...
    # Intelligent filtering (Spotify-like): 
    # Remove tracks longer than 15 minutes (900s) to avoid 1hr+ loops
    # Remove tracks already played
    filtered_playlist = []
    for t in playlist:
        if t["id"] in played_ids:
            continue
        try:
            # simple duration check if duration is like '5:30'
            parts = t.get("duration", "").split(":")
            if len(parts) >= 3: # HH:MM:SS
                if int(parts[0]) > 0: # >1 hour
                    continue
        except Exception:
            pass
        filtered_playlist.append(t)

    unplayed = filtered_playlist
...
async def cache_autoplay(chat_id: int, last: dict):
    """Background task to pre-fetch recommendations while a song plays."""
    try:
        # Cancel any existing pre-fetch task for this chat
        if chat_id in _prefetch_tasks and not _prefetch_tasks[chat_id].done():
        ...
        async def _run_cache():
            ...
            state = autoplay_state.get(chat_id, {})
            if state.get("seed_id") == last_id and state.get("playlist"):
            ...
            if playlist:
                played_ids = state.get("played_ids", set())
                autoplay_state[chat_id] = {
                    "seed_id": last_id,
                    "playlist": playlist,
                    "index": 0,
                ...
                if next_id:
                    from Dev import yt
                    await yt.download(next_id, video=False)
                
        _prefetch_tasks[chat_id] = asyncio.create_task(_run_cache())
    except Exception:
        pass
